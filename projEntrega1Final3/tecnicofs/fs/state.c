...
int inode_create(inode_type n_type) {
    ...
    // locking before the for cicle so there are no constant locking and unlocking
    // which could slow down significantly the program
...
int inode_delete(int inumber) {
    ...
    // locking before the for cicle so there are no constant locking and unlocking
    // which could slow down significantly the program. Can only free the lock after all the content is deleted

    if (inode_table[inumber].i_size > 0) {

        int blocksToFree = (int) inode_table[inumber].i_size / BLOCK_SIZE;
        ...
        for (int i = 0; i < blocksToFree; i++) {
            if (i < DIRECT_DATA_BLOCKS) {
                ...
                int index = i - DIRECT_DATA_BLOCKS;
                int *indirectBlock = data_block_get(inode_table[inumber].i_indirect_data_block);
                if (data_block_free(indirectBlock[index]) == -1) {
                    data_block_free(inode_table[inumber].i_indirect_data_block);
                    pthread_mutex_unlock(&mutex);
                    return -1;
                }
            }
        }
        if (blocksToFree > DIRECT_DATA_BLOCKS) {
...
int add_dir_entry(int inumber, int sub_inumber, char const *sub_name) {
    // lock the mutex to protect inode_table
...
/*
 * Allocated all direct data blocks
 * Returns: 0 if successful, -1 otherwise
 */
int direct_blocks_alloc(inode_t* inode, int start, int n_blocks) {
...
int data_blocks_free(inode_t *inode) {
    int n_blocks = (int) inode->i_size / BLOCK_SIZE;
    int val;
    ...
    for (int i = 0; i <= n_blocks; i++) {
        int block_number = i;
        if (i >= DIRECT_DATA_BLOCKS) {
            block_number = i - DIRECT_DATA_BLOCKS;
            int *indirectBlock = data_block_get(inode->i_indirect_data_block);
            val = data_block_free(indirectBlock[block_number]);
        }
        else {
            val = data_block_free(inode->i_data_block[block_number]);
        }
        if (val == -1) {
            return -1;
        }
